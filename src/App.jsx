...
const STAP_NAMEN = ["Bedrijfsprofiel", "Doelgroep", "Segmenten & Inzichten", "Pijnpunten", "Strategie-matrix", "Campagne-insteek", "Advertenties", "Meta Setup", "Campagne Evaluatie"];
...
function Stap2({ bedrijf, onCsvData, onNext, onHelp, onBack }) {
  ...
  const analyseer = async () => {
    ...
    setAnalyseStappen([]);
    try {
      setAnalyseStappen([{ tekst: "Data verwerken en bedrijfsprofiel opstellen…", klaar: false }]);
      const d = csvText || handmatig || "(geen data)";
      await new Promise(r => setTimeout(r, 300));
      setAnalyseStappen([
        { tekst: "Data verwerkt ✓", klaar: true },
        { tekst: "Micro-segmenten genereren op basis van doelgroep…", klaar: false },
      ]);
      const raw = await callClaude(
...
Alleen de JSON array, geen uitleg.`
      ...
      setAnalyseStappen([
        { tekst: "Data verwerkt ✓", klaar: true },
        { tekst: "Segmenten gegenereerd ✓", klaar: true },
        { tekst: "Resultaten verwerken…", klaar: false },
      ]);
  ...
  return (
    <Card>
      <StepTitle emoji="🎯" title="Doelgroepanalyse"
        sub="Beschrijf je doelgroep. Je co-pilot bouwt er micro-segmenten uit."
...
function Stap3({ bedrijf, segmenten, setSegmenten, onNext, onHelp, onBack }) {
  ...
  const [concResultaat, setConcResultaat] = useState(""); // toon gevonden concurrenten/pijnpunten
  ...
  return (
    <Card>
      <StepTitle emoji="👥" title="Jouw 4 micro-segmenten"
        sub="Je co-pilot heeft 4 micro-segmenten gebouwd. Voeg reviews toe voor diepere pijnpuntenanalyse."
        onHelp={onHelp} heeftNaam={!!bedrijf.naam} />
      ...
      {reviewStappen.length > 0 && renderStappen(reviewStappen, C.groen)}
      ...
      {concStappen.length > 0 && renderStappen(concStappen, C.groen)}
      {/* Gevonden concurrenten + pijnpunten tonen */}
      {concResultaat && (
        <div style={{ background: C.bgMid, border: `1px solid ${C.borderGreen}`, borderRadius: 12, padding: "16px 20px", marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
            <div style={{ fontFamily: font.body, fontWeight: 700, fontSize: 12, color: C.groen, textTransform: "uppercase", letterSpacing: "1.2px" }}>
              🏆 Gevonden concurrenten &amp; klantpijnpunten
            </div>
            <button onClick={() => setConcResultaat("")} style={{ background: "transparent", border: "none", color: C.muted, cursor: "pointer", fontSize: 16, lineHeight: 1 }}>×</button>
          </div>
          <div style={{ fontFamily: font.body, fontSize: 13, color: C.textSoft, lineHeight: 1.8, whiteSpace: "pre-wrap", maxHeight: 280, overflowY: "auto" }}>
            {concResultaat}
          </div>
          <div style={{ marginTop: 10, fontSize: 11, color: C.muted, fontFamily: font.body }}>
            ℹ️ Deze data wordt automatisch meegenomen bij de pijnpuntenanalyse.
          </div>
        </div>
      )}
  ...
  return (
    <Card>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        <StepTitle emoji="🎯" title="Selecteer pijnpunten" sub="Kies de 6 pijnpunten die het sterkst resoneren met jouw doelgroep." onHelp={onHelp} heeftNaam={!!bedrijf?.naam} />
...
function Stap5({ segmenten, pijnpunten, gekozenPijnpunten, combinaties, setCombinaties, onNext, bedrijf, onHelp }) {
  ...
  return (
    <Card>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        <StepTitle emoji="🔢" title="Segment × Pijnpunt matrix" sub="Je co-pilot combineert segmenten en pijnpunten. Kies de 6 sterkste combinaties." onHelp={onHelp} heeftNaam={!!bedrijf?.naam} />
...
function Stap6({ bedrijf, combinaties, segmenten, pijnpunten, onNext, onBack, onHelp }) {
  ...
  return (
    <Card>
      <StepTitle emoji="🚀" title="Kies je campagne-insteek" sub="Je co-pilot stelt de beste campagneformules voor op basis van jouw profiel." onHelp={onHelp} heeftNaam={!!bedrijf.naam} />
      <SectorBalk aanbod={bedrijf.aanbod} />
      {loading && (
        <div style={{ marginBottom: 24 }}>
...
function CombinatieTeksten({ bedrijf, seg, pijnpunt, campagne }) {
  ...
  const genereerAds = async () => {
    ...
    try {
      ...
      const prompt1 = context + ". Schrijf 5 Meta Ads advertentieteksten (1 per hook-type: Emotioneel, Rationeel, Direct probleem, Urgentie, Droom). Elke tekst: hook + oplossing + CTA. Voeg per tekst ook een veld 'waarom' toe: 1 zin waarom deze hook werkt voor dit segment en pijnpunt. Geef output als JSON object met veld teksten (array van objecten met hook_type, tekst en waarom).";
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {(ads.teksten ?? []).map((t, i) => (
                <div key={i} style={{ background: C.bgMid, borderRadius: 12, padding: 20, border: `1px solid ${C.border}` }}>
                  ...
                  <p style={{ fontFamily: font.body, fontSize: 14, lineHeight: 1.75, margin: "0 0 12px 0", whiteSpace: "pre-wrap", color: C.textSoft }}>{(t.tekst ?? "").replace(/\\n/g, "\n")}</p>
                  {t.waarom && (
                    <div style={{ display:"flex", alignItems:"flex-start", gap:8, background:C.groenLight, border:`1px solid ${C.borderGreen}`, borderRadius:8, padding:"8px 12px" }}>
                      <span style={{ fontSize:13, flexShrink:0 }}>💡</span>
                      <span style={{ fontFamily:font.body, fontSize:12, color:C.groenDim, lineHeight:1.6, fontStyle:"italic" }}>
                        <strong style={{ fontStyle:"normal", color:C.groen }}>Waarom dit werkt:</strong> {t.waarom}
                      </span>
                    </div>
                  )}
                </div>
  ...
  const evalueer = async () => {
    ...
    try {
      ...
      const prompt = "Evalueer deze Meta Ads campagnedata voor " + bedrijf.naam + "."
        ...
        + " CAMPAGNE: [naam]"
        + " BESLISSING: [STOP DIRECT of OPTIMALISEER of BLIJVEN LOPEN of OPSCHALEN]"
        + " PRIORITEIT: [1 of 2 of 3]"
        + " VERTROUWEN: [LAAG of MATIG of STERK]"
        + " REDEN: [waarom deze beslissing in 1-2 zinnen]"
        + " CIJFERS: [de belangrijkste metric die de beslissing ondersteunt, bv. CTR 0.4%, CPC €3.20, 0 leads]"
        + " ACTIE: [1 concrete implementeerbare stap]"
        + " ---"
        + " Daarna: ===BUDGET GUARDIAN [max 3 waarschuwingen]"
        + " ===CREATIEVE FATIGUE SIGNALEN: [komma-lijst] DIAGNOSE: [1 zin] NIEUWE HOOK: [voorstel]"
        + " ===DAGELIJKSE SAMENVATTING [uitgebreidere versie van owner summary]"
        + " Schrijf in het Nederlands. Geen jargon. Wees concreet.";
...
<script>setTimeout(()=>window.print(),500);<\/script>
  ...
  const renderEvaluatie = (tekst) => {
    ...
    const CampagneKaart = ({ blok, idx }) => {
      ...
      // Prioriteit config
      const prioConfig = blok.prioriteit === 1
        ? { label: "Eerst aanpakken", bg: C.groen, color: "#fff" }
        : blok.prioriteit === 3
        ? { label: "Mag wachten", bg: C.bgWarm, color: C.muted }
        : null;

      // Vertrouwen config
      const vertrConfig = blok.vertrouwen === "STERK"
        ? { label: "✓ Voldoende data", bg: "#E8F5EE", color: C.groen, brd: C.borderGreen }
        : blok.vertrouwen === "MATIG"
        ? { label: "~ Indicatief", bg: "#FEF9EC", color: "#8A6200", brd: "#E8D080" }
        : blok.vertrouwen === "LAAG"
        ? { label: "⚠ Weinig data", bg: "#FEF0F0", color: "#B03A2E", brd: "#F0B0A0" }
        : null;

      return (
        <div style={{ borderRadius:14, marginBottom:16, overflow:"hidden", border:`2px solid ${kl.dot}`, boxShadow:`0 2px 12px ${kl.dot}20` }}>

          {/* ── TITEL BALK ── */}
          <div style={{ background:kl.bg, padding:"14px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:10, borderBottom:`1px solid ${kl.brd}` }}>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              {/* Grote gekleurde bol */}
              <div style={{ width:20, height:20, borderRadius:"50%", background:kl.dot, flexShrink:0, boxShadow:`0 0 0 5px ${kl.dot}22` }} />
              <span style={{ fontFamily:font.display, fontWeight:700, fontSize:16, color:C.text }}>{blok.naam || "Campagne"}</span>
            </div>
            {/* Beslissing badge - rechts */}
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              {prioConfig && <span style={{ background:prioConfig.bg, color:prioConfig.color, fontSize:10, fontWeight:800, padding:"3px 10px", borderRadius:4, fontFamily:font.body, letterSpacing:"1px", textTransform:"uppercase" }}>{prioConfig.label}</span>}
          ...
          {/* ── BODY: 3 duidelijke blokken ── */}
          <div style={{ background:"white", padding:"0" }}>

            {/* Blok 1: WAAROM */}
            {(blok.reden || blok.cijfers) && (
              <div style={{ padding:"12px 20px", borderBottom:`1px solid ${C.border}`, display:"flex", gap:14, alignItems:"flex-start" }}>
                <div style={{ width:28, height:28, borderRadius:7, background:"#F0F4FF", display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, flexShrink:0, marginTop:1 }}>📋</div>
                <div style={{ flex:1 }}>
                  <div style={{ fontFamily:font.body, fontWeight:700, fontSize:10, color:C.muted, textTransform:"uppercase", letterSpacing:"1.5px", marginBottom:5 }}>Waarom deze beslissing</div>
                  {blok.reden && <div style={{ fontFamily:font.body, fontSize:13, color:C.textSoft, lineHeight:1.7, marginBottom: blok.cijfers ? 6 : 0 }}>{blok.reden}</div>}
                  {blok.cijfers && (
                    <div style={{ display:"inline-flex", alignItems:"center", gap:6, background:kl.bg, border:`1px solid ${kl.brd}`, borderRadius:6, padding:"4px 10px" }}>
                      <span style={{ fontFamily:"monospace", fontWeight:700, fontSize:12, color:kl.dot }}>📊 {blok.cijfers}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            {/* Blok 2: ACTIE */}
            {blok.actie && (
              <div style={{ padding:"12px 20px", borderBottom: vertrConfig ? `1px solid ${C.border}` : "none", display:"flex", gap:14, alignItems:"flex-start" }}>
                <div style={{ width:28, height:28, borderRadius:7, background:C.groenLight, display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, flexShrink:0, marginTop:1 }}>⚡</div>
                <div style={{ flex:1 }}>
                  <div style={{ fontFamily:font.body, fontWeight:700, fontSize:10, color:C.muted, textTransform:"uppercase", letterSpacing:"1.5px", marginBottom:5 }}>Concrete actie</div>
                  <div style={{ fontFamily:font.body, fontSize:14, color:C.navy, fontWeight:600, lineHeight:1.6 }}>{blok.actie}</div>
                </div>
              </div>
            )}
            ...
            {/* Blok 3: VERTROUWEN */}
            {vertrConfig && (
              <div style={{ padding:"8px 20px", background:vertrConfig.bg, display:"flex", alignItems:"center", gap:8 }}>
                <span style={{ fontFamily:font.body, fontSize:11, color:vertrConfig.color, fontWeight:700 }}>{vertrConfig.label}</span>
                <span style={{ fontFamily:font.body, fontSize:11, color:vertrConfig.color, opacity:.7 }}>· basis voor dit advies</span>
              </div>
            )}
          </div>
        </div>
    ...
    return (
      <div>
        ...
        {/* DAILY OWNER SUMMARY — bovenaan */}
        {ownerSummary && (
          <div style={{ background: C.groen, borderRadius:12, padding:"18px 22px", marginBottom:20 }}>
        ...
        {campagnes.length > 0 && <>
          <div style={{ fontFamily:font.body, fontWeight:700, fontSize:11, color:C.muted, textTransform:"uppercase", letterSpacing:"1px", marginBottom:10 }}>
            Evaluatie per campagne ({campagnes.length}) — gesorteerd op prioriteit
          </div>
    ...
    <div>
      ...
      <div style={{ background:C.card, borderRadius:18, border:`1px solid ${C.border}`, padding:"28px 32px", boxShadow:C.shadow, marginBottom:16 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:6 }}>
          ...
          <div>
            <h2 style={{ fontFamily:font.display, fontWeight:700, fontSize:22, margin:0, color:C.text }}>Campagne Evaluatie</h2>
            <p style={{ color:C.muted, fontSize:13, fontFamily:font.body, margin:0 }}>
              Je co-pilot analyseert je campagnes en zegt wat je vandaag doet, optimaliseert en stopzet
            </p>
          </div>
        </div>
        {playbook && (
    ...
    <div style={{ background:"linear-gradient(90deg,#1D4D33,#2C6E49)", borderBottom:"3px solid #4ADE80", padding:"16px 28px", position:"sticky", top:64, zIndex:99 }}>
      <div style={{ maxWidth:900, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div>
            <div style={{ fontFamily:font.display, fontWeight:700, fontSize:16, color:"#fff" }}>Gratis proefrun voltooid!</div>
            <div style={{ fontSize:12, color:"rgba(255,255,255,.8)", fontFamily:font.body, marginTop:2 }}>Je hebt de volledige co-pilot ervaren. Upgrade voor onbeperkt gebruik.</div>
          </div>
        </div>
        <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
          <button onClick={onReset} style={{ background:"transparent", border:"1px solid rgba(255,255,255,.4)", borderRadius:8, padding:"8px 16px", color:"rgba(255,255,255,.7)", fontFamily:font.body, fontSize:12, cursor:"pointer" }}>
            ↺ Opnieuw proberen
          </button>
          <a href="https://verdify.eu" target="_blank" rel="noopener" style={{ background:"#4ADE80", color:"#1D4D33", fontFamily:font.body, fontWeight:800, fontSize:13, padding:"10px 22px", borderRadius:9, textDecoration:"none", whiteSpace:"nowrap", display:"inline-block" }}>
    ...
    <div style={{ background:"linear-gradient(90deg,#4A1A8A,#7B3FD0)", borderBottom:"3px solid #C8A8F0", padding:"12px 28px", position:"sticky", top:64, zIndex:99 }}>
      <div style={{ maxWidth:900, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
        <div style={{ display:"flex", alignItems:"center", gap:14 }}>
          ...
          <div style={{ fontFamily:font.body, fontSize:13, color:"rgba(255,255,255,.9)" }}>
            <strong style={{ color:"#fff" }}>Stap {stapBereikt}/9</strong> — Doorloop alle stappen voor een volledige campagne-evaluatie
          </div>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <div style={{ width:140, height:8, background:"rgba(255,255,255,.2)", borderRadius:4, overflow:"hidden" }}>
            <div style={{ width:pct+"%", height:"100%", background:"#C8A8F0", borderRadius:4, transition:"width .5s ease" }} />
          </div>
          <span style={{ fontFamily:font.body, fontSize:12, color:"rgba(255,255,255,.9)", fontWeight:700, minWidth:32 }}>{pct}%</span>
        </div>
      </div>
    </div>
  );
